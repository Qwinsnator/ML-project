...
from sklearn.metrics import recall_score, make_scorer, roc_curve, roc_auc_score
...
def grid_tune_metrics(X_sel, y, cv):
    ...
    # Single ROC plot for all classifiers
    fig, ax = plt.subplots(figsize=(10, 8))
    colors = plt.cm.tab10(np.linspace(0, 1, len(clfs)))
    
    for idx, (name, clf_base) in enumerate(clfs.items()):
        print(f"Tuning {name}...")
        ...
        if hasattr(best_clf, 'predict_proba'):
            y_score = cross_val_predict(best_clf, X_sel, y, cv=cv, method='predict_proba')[:, 1]
        elif hasattr(best_clf, 'decision_function'):
            y_score = cross_val_predict(best_clf, X_sel, y, cv=cv, method='decision_function')
        else:
            y_score = cross_val_predict(best_clf, X_sel, y, cv=cv, method='predict')
        ...
        auc_cv = cross_val_score(best_clf, X_sel, y, cv=cv, scoring='roc_auc')
    ...
    plt.tight_layout()
    plt.savefig('results_grid/combined_roc_curves.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("Saved combined ROC: results_grid/combined_roc_curves.png")
