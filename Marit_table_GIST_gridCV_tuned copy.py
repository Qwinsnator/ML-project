...
def grid_tune_metrics(X_sel, y, cv):
    ...
    # Formatteer de kolommen naar de "Mean ± Std" weergave
    summary_df = pd.DataFrame()
    summary_df['Classifier'] = df_results['classifier']
    summary_df['Recall']     = df_results.apply(lambda r: f"{r['recall_mean']:.2f} ± {r['recall_std']:.2g}", axis=1)
    summary_df['AUC']        = df_results.apply(lambda r: f"{r['auc_mean']:.2f} ± {r['auc_std']:.2g}", axis=1)
    summary_df['Accuracy']   = df_results.apply(lambda r: f"{r['acc_mean']:.2f} ± {r['acc_std']:.2g}", axis=1)
    summary_df['F1-score']   = df_results.apply(lambda r: f"{r['f1_mean']:.2f} ± {r['f1_std']:.2g}", axis=1)
    summary_df['Specificity']= df_results.apply(lambda r: f"{r['spec_mean']:.2f} ± {r['spec_std']:.2g}", axis=1)

    # Print de tabel mooi in de console
    print("\n=== EINRESULTAAT TABEL ===")
    print(summary_df.to_string(index=False))

    # Sla op als CSV (voor Excel)
    summary_df.to_csv('mooie_tabel_resultaten.csv', index=False)
