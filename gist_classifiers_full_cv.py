"""
GIST Classifier Analysis - Full Feature Set with Cross-Validation
Applies the E1.2 Basic Classifiers code to the GIST radiomic features dataset.
Uses full feature set and cross-validation for realistic performance estimates.
"""
...
def load_gist_data_full():
    """
    Load and preprocess the GIST radiomic features dataset - full feature set.
    """
    print("Loading GIST radiomic features data (full feature set)...")
    df = load_data()
...
def run_classifiers_with_cv(X, y, output_dir, n_splits=5):
    ...
    for idx, (clf_name, clf) in enumerate(classifiers.items()):
        ...
        bp = ax.boxplot(cv_data, labels=['Acc', 'F1', 'AUC', 'Prec', 'Rec'])
...
def run_with_feature_selection(X, y, output_dir, n_splits=5):
    ...
    """
    print(f"\n{'='*60}")
    print(f"Running with Feature Selection + Cross-Validation")
    print(f"{'='*60}")
...
def main():
    ...
    fs_results = run_with_feature_selection(X, y, output_dir)
